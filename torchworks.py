...
def get_class_distribution(obj, n_classes):
    count_dict = {}
...
def get_class_weights(classes, train_dataset, y_train):
    ...
    class_count = [i for i in get_class_distribution(y_train, len(classes)).values()]
...
def transform_matrices(dfs, rows_ex, cols_ex):
    ...
    classes = list(set([x.split(".")[0] for x in columns]))
    ...
    idx2class = {i: classes[i] for i in range(0, len(classes))}
    class2idx = {v: k for k, v in idx2class.items()}
    outputs = [class2idx[x] for x in outputs]
...
def train_mini(lang, bert=False, rows_ex=None, cols_ex=None, wanted=None, name="miniNN",
                 epochs=None, batch_size=64, lr=0.001, val_size=0.2):
    ...
    inputs, outputs, idx2class, class2idx = transform_matrices(pd_csvs, rows_ex, cols_ex)
